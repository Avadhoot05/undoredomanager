...
{
    constructor(undoCb, redoCb, limit = 100)
    {
        this.limit = limit;
        this.stkUndo = [];
        this.stkRedo = [];
    ...
    AddCommandToUndoStack(undoCommand)
    {
        ...
        //     undoCb: //function or null (if null fallback undo callback is called)
        //     redoCb: //function or null (if null fallback undo callback is called)
    ...
    OnUndo(event)
    {
        ...
        let {undoCb, _, context, preventDefault} = this.mapCallbacks.get(id);
        ...
        this.AddCommandToRedoStack(undoCommand);    
    }
    ...
    OnRedo(event)
    {
        let redoCommand = this.stkRedo.pop();
        if(!redoCommand)
            return;
